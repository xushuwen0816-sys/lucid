...
const IntentView: React.FC<IntentViewProps> = ({ state, setState, onComplete }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full h-full flex flex-col overflow-hidden font-serif">
        ...
        {state.step === 'input' && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-fade-in">
            <div className="w-full max-w-xl space-y-6 text-center">
              <h3 className="text-3xl font-serif text-white/90 leading-tight">此刻，<br/>你想显化什么？</h3>
              <textarea
                className="w-full bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 text-lg text-center focus:ring-1 focus:ring-lucid-glow/30 focus:outline-none transition-all resize-none placeholder-white/20 font-serif leading-relaxed text-lucid-text"
                rows={4}
            ...
            <Button onClick={handleStartDeepDive} disabled={!state.wishInput} variant="primary" className="rounded-full px-10 py-3 text-lg shadow-xl shadow-lucid-glow/10">
               开启对话 <Sparkles className="w-4 h-4 ml-2" />
            </Button>
          </div>
        )}
          ...
          <div className="flex flex-col h-full bg-white/[0.02] rounded-[2rem] border border-white/5 relative overflow-hidden shadow-inner max-h-[70vh]">
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar mb-24">
              {state.messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                  <div className={`max-w-[85%] p-5 rounded-2xl text-[15px] font-serif leading-loose tracking-wide shadow-sm ${
                    ...
                    {msg.role === 'model' && <div className="text-[10px] font-sans text-lucid-accent mb-1 uppercase tracking-widest opacity-80">LUCID</div>}
                    {msg.text}
                  </div>
                </div>
              ))}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 bg-lucid-bg/95 backdrop-blur-2xl border-t border-white/5 z-20">
               ...
               {state.messages.length > 1 && (
                 <div className="flex justify-center mb-3 animate-fade-in">
                    <Button 
                        ...
                        disabled={isLoading}
                        variant="glass" 
                        className="rounded-full px-6 py-1.5 text-xs border-lucid-glow/30 text-lucid-glow hover:bg-lucid-glow/10 min-w-[240px]"
                    >
                       {isLoading ? (
               ...
               <div className="flex gap-3 relative items-end">
                 <textarea
                   className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white focus:outline-none focus:bg-white/10 font-serif resize-none h-12 text-sm"
                   placeholder="回复以继续挖掘..."
                   value={state.wishInput}
                   onChange={(e) => setState(prev => ({ ...prev, wishInput: e.target.value }))}
                   onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSendMessage())}
                 />
                 <Button onClick={handleSendMessage} variant="primary" className="h-12 w-12 !p-0 rounded-full" disabled={!state.wishInput.trim()}>
        ...
        {state.step === 'affirmation-select' && (
           <div className="flex flex-col gap-4 animate-fade-in pb-32 max-w-2xl mx-auto">
               <div className="text-center mb-6">
                   ...
                   <p className="text-lucid-dim text-xs mt-2 font-serif tracking-wider">点击下一步，我们将把这些频率植入潜意识。</p>
               </div>
               <div className="grid gap-3">
                   {state.generatedAffirmations.map((aff, i) => (
                       <Card 
                          key={i} 
                          className="border border-white/5 hover:bg-white/5 transition-all bg-white/[0.02] hover:border-lucid-glow/20 p-5"
                       >
                           <span className={`text-[9px] uppercase tracking-widest mb-1 block font-sans ${
                           ...
                           <p className="text-lg font-serif text-white leading-relaxed">"{aff.text}"</p>
                       </Card>
                   ))}
               </div>
           </div>
        ...
        {state.step === 'voice-gen' && (
            <div className="flex flex-col gap-6 animate-fade-in items-center justify-center min-h-[50vh]">
                <div className="text-center">
                   ...
                   <p className="text-lucid-dim text-xs mt-1 font-serif">选择 AI 导读或亲自录制肯定语</p>
                </div>
                ...
                <div className="flex bg-white/5 rounded-full p-1 border border-white/5">
                    <button 
                        ...
                        className={`px-6 py-2 rounded-full text-xs font-serif transition-all ${voiceMode === 'ai' ? 'bg-lucid-glow text-lucid-bg' : 'text-lucid-dim'}`}
                    >
                        AI 导读
                    </button>
                    <button 
                        onClick={() => { setVoiceMode('user'); stopAudio(); setState(prev => ({...prev, generatedVoiceAudio: null})) }}
                        className={`px-6 py-2 rounded-full text-xs font-serif transition-all ${voiceMode === 'user' ? 'bg-lucid-glow text-lucid-bg' : 'text-lucid-dim'}`}
                    >
                        亲自录制
                    </button>
                </div>
                
                <div className="w-full max-w-md p-8 bg-white/[0.03] rounded-[2rem] border border-white/5 flex flex-col items-center gap-6 shadow-xl relative overflow-hidden">
                       ...
                       <>
                           <div className="flex gap-2 justify-center mb-2">
                                {['Kore', 'Fenrir', 'Puck'].map(v => (
                                    ...
                                    className={`px-4 py-1.5 rounded-full text-[10px] font-serif transition-all border ${state.selectedVoice === v ? 'bg-white text-black border-white' : 'text-lucid-dim border-transparent hover:border-white/20'}`}
                                    >
                            ...
                            {state.generatedVoiceAudio ? (
                                <div className="flex items-center gap-4 w-full">
                                    ...
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <div className="text-[10px] text-lucid-dim font-serif uppercase tracking-wider flex justify-between">
                                ...
                                <Button onClick={handleVoiceGen} disabled={isLoading} variant="outline" className="w-full py-3 rounded-xl border-white/10 hover:bg-white/5 font-serif">
                           ...
                           {state.generatedVoiceAudio ? (
                               <div className="flex items-center gap-4 w-full">
                                   ...
                                   <div className="flex-1 text-center">
                                       <div className="text-xs text-emerald-400 mb-2">录音已完成</div>
                                       <button onClick={() => setState(prev => ({...prev, generatedVoiceAudio: null}))} className="text-[10px] text-lucid-dim underline hover:text-white">重新录制</button>
                                   </div>
                               </div>
                           ) : (
                               <>
                                   <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-all duration-500 ${isRecording ? 'bg-red-500/20 shadow-[0_0_30px_rgba(239,68,68,0.4)] scale-110' : 'bg-white/5'}`}>
                                       <button 
                                          onClick={isRecording ? stopRecording : startRecording}
                                          className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${isRecording ? 'bg-red-500 text-white' : 'bg-lucid-glow text-lucid-bg hover:bg-white'}`}
                                       >
                                           {isRecording ? <div className="w-5 h-5 bg-white rounded-sm"></div> : <Mic className="w-6 h-6" />}
                                       </button>
                                   </div>
                                   <p className="text-xs text-lucid-dim">{isRecording ? '正在录制...再次点击结束' : '点击开始朗读肯定语'}</p>
                                   <div className="text-[10px] text-stone-500 border-t border-white/5 pt-3 mt-2 w-full text-center">
        ...
        {state.step === 'music-gen' && (
            <div className="flex flex-col gap-6 animate-fade-in pb-32 max-w-3xl mx-auto">
                <div className="text-center">
                   ...
                   <p className="text-lucid-dim text-xs mt-1 font-serif">选择背景音乐风格</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    {['超柔和 Ambient', '脑波 Binaural', '白噪音 Rain', 'Gameboy 8bit', '女巫梦境 Witch', '水晶疗愈 Crystal'].map(style => (
                        <button
                          ...
                          className={`p-4 rounded-2xl border text-left transition-all duration-300 ${state.selectedMusicStyle === style ? 'bg-lucid-glow/10 border-lucid-glow text-white shadow-[0_0_20px_rgba(253,186,116,0.15)] scale-[1.02]' : 'bg-white/5 border-white/5 text-lucid-dim hover:bg-white/10'}`}
                        >
                            <Music className={`w-4 h-4 mb-2 ${state.selectedMusicStyle === style ? 'text-lucid-glow' : 'opacity-30'}`}/>
                            <span className="text-xs font-serif block tracking-wide">{style}</span>
                        </button>
                    ))}
                </div>
                
                <div className="flex flex-col items-center justify-center p-6 bg-black/10 rounded-[2rem] border border-white/5 space-y-4">
                    {!state.generatedMusicAudio ? (
                         <Button onClick={handleMusicGen} disabled={!state.selectedMusicStyle || isLoading} variant="primary" className="w-full max-w-xs py-3 rounded-full">
                             {isLoading ? <LoadingSpinner/> : '生成预览 (15秒)'}
                         </Button>
                    ) : (
                        <div className="flex flex-col items-center gap-4 w-full">
                           <div className="text-xs text-lucid-glow flex items-center gap-2 bg-lucid-glow/10 px-3 py-1 rounded-full font-serif">
                               <Check className="w-3 h-3"/> 预览已生成
                           </div>
                           <div className="flex gap-3">
                                <Button onClick={() => playBuffer(state.generatedMusicAudio!, true)} variant="glass" className="rounded-full !p-4">
                                    {isPlaying ? <Pause className="w-5 h-5 fill-current"/> : <Play className="w-5 h-5 fill-current ml-1"/>}
                                </Button>
                                <Button onClick={handleMusicGen} variant="outline" className="px-6 rounded-full text-xs">重新生成</Button>
                           </div>
                        </div>
                    )}
                </div>
        ...
        {state.step === 'subliminal-mix' && (
            <div className="flex flex-col items-center justify-center gap-8 animate-fade-in text-center pb-32">
                <div className="relative mt-4 group">
                    ...
                    <div className="relative z-10 w-24 h-24 rounded-full border border-white/10 bg-white/5 flex items-center justify-center backdrop-blur-sm">
                        <Layers className="w-10 h-10 text-white/90" />
                    </div>
                </div>
                
                <div>
                    <h3 className="text-2xl font-serif text-white mb-1">最终融合</h3>
                    <p className="text-lucid-dim font-serif text-xs">合成 3 分钟专属潜意识音频</p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 w-full max-w-md">
                    {[
                        ...
                        <button
                            ...
                            className={`p-4 rounded-2xl border transition-all duration-300 ${state.mixingMode === mode.id ? 'bg-white/10 border-lucid-glow text-white scale-105 shadow-lg' : 'bg-transparent border-white/10 text-lucid-dim hover:bg-white/5'}`}
                        >
                            <span className="block font-serif mb-1 text-sm">{mode.label}</span>
                            <span className="text-[9px] opacity-60 block tracking-wider font-sans">{mode.desc}</span>
                        </button>
                    ))}
                </div>

                <div className="flex flex-col w-full max-w-xs gap-3">
                    <Button onClick={handleMixing} disabled={isLoading} variant="glass" className="w-full py-3 text-sm rounded-full">
                    ...
                    {state.finalSubliminalAudio && (
                        ...
                        }} variant="outline" className="w-full py-3 border-white/10 hover:border-white/30 rounded-full text-sm">
      ...
      {state.step !== 'input' && state.step !== 'deep-dive' && (
          <div className="flex-shrink-0 p-4 border-t border-white/5 bg-lucid-bg/80 backdrop-blur-xl flex justify-between items-center z-50">
             <Button 
                onClick={() => {
                   stopAudio();
             ...
             {state.step === 'affirmation-select' && (
                 <Button onClick={() => { stopAudio(); setState(prev => ({ ...prev, step: 'voice-gen' })); }} variant="primary" className="rounded-full px-6">
                    下一步 <ArrowRight className="w-4 h-4 ml-2" />
                 </Button>
             )}
             {state.step === 'voice-gen' && (
                 <Button disabled={!state.generatedVoiceAudio} onClick={() => { stopAudio(); setState(prev => ({...prev, step: 'music-gen'})); }} variant="primary" className="rounded-full px-6">
                    下一步 <ArrowRight className="w-4 h-4 ml-2"/>
                 </Button>
             )}
             {state.step === 'music-gen' && (
                 <Button disabled={!state.generatedMusicAudio} onClick={() => {stopAudio(); setState(prev => ({...prev, step: 'subliminal-mix'}))}} variant="primary" className="rounded-full px-6">
                    下一步 <ArrowRight className="w-4 h-4 ml-2"/>
                 </Button>
             )}
             {state.step === 'subliminal-mix' && (
                  <Button onClick={handleSave} disabled={!state.finalSubliminalAudio || isLoading} variant="primary" className="rounded-full px-6">
